...
export default function UnitPage({ params }: UnitPageProps) {
    ...
    const handleSaveGeneral = () => {
        updateUnitMutation.mutate({ id: unitId, name: unitName, timezone: unitTimezone }, {
            onSuccess: () => {
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                <TabsList>
                    ...
                    <PermissionGuard permissions={['UNIT_GRID_MANAGE']} unitId={unitId}>
                        <TabsTrigger value="grid-configuration">{t('grid_configuration.title', { defaultValue: 'Grid Configuration' })}</TabsTrigger>
                    </PermissionGuard>
                    <PermissionGuard permissions={['UNIT_SETTINGS_MANAGE']} unitId={unitId}>
                        <TabsTrigger value="counters">{t('counters.title')}</TabsTrigger>
                    </PermissionGuard>
                    <PermissionGuard permissions={['UNIT_TICKET_SCREEN_MANAGE']} unitId={unitId}>
                ...
                <TabsContent value="services" className="mt-6">
                    <PermissionGuard permissions={['UNIT_SERVICES_MANAGE']} unitId={unitId} fallback={<div>{t('access_denied')}</div>}>
                        <UnitServicesManager unitId={unitId} />
                    </PermissionGuard>
                </TabsContent>
                <TabsContent value="grid-configuration" className="mt-6">
                    <PermissionGuard permissions={['UNIT_GRID_MANAGE']} unitId={unitId} fallback={<div>{t('access_denied')}</div>}>
