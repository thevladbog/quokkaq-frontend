    ...
    return (
        <div className="container mx-auto p-4">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold">{t('units.title', { defaultValue: 'Units' })}</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {units.map((unit) => (
                    <Card key={unit.id} className="cursor-pointer hover:bg-accent transition-colors" onClick={() => router.push(`/admin/units/${unit.id}`)}>
                        <CardHeader>
            ...
            <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
                <DialogContent className="sm:max-w-[425px]">
                    ...
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setCreateDialogOpen(false)}>{t('general.cancel')}</Button>
                        <Button onClick={handleCreateUnit} disabled={createUnitMutation.isPending}>
                            {createUnitMutation.isPending ? t('general.creating') : t('units.add')}
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
