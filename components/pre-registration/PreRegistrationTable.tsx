...
export function PreRegistrationTable({ unitId, onEdit }: PreRegistrationTableProps) {
    ...
    if (!preRegistrations || preRegistrations.length === 0) {
        return <div className="text-center p-8 text-muted-foreground">{t('no_records')}</div>;
    }

    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'created':
                return <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">{t('status.created')}</Badge>;
            case 'ticket_issued':
                return <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">{t('status.ticket_issued')}</Badge>;
            case 'canceled':
                return <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200">{t('status.canceled')}</Badge>;
            default:
