...
export function PreRegRedemptionModal({ isOpen, onClose, unitId, onSuccess }: PreRegRedemptionModalProps) {
            ...
            <DialogContent className="sm:max-w-[600px] p-0 overflow-hidden">
                <div className="p-8 pb-0">
                    <DialogHeader className="mb-6">
                        <DialogTitle className="text-center text-3xl">{t('enter_code', { defaultValue: 'Enter Code' })}</DialogTitle>
                    </DialogHeader>
                    ...
                    <div className="flex justify-center mb-8 " >
                        <Input
                            value={code}
                            readOnly
                            className="text-center !text-8xl md:!text-6xl tracking-[0.5em] !h-32 w-full font-mono font-bold"
                            placeholder="------"
                        />
                    </div>

                    {error && (
                        <div className="text-destructive text-center mb-6 text-lg font-medium bg-destructive/10 p-4 rounded-lg">
                            {error}
                        </div>
                    )}
                </div>
                ...
                <div className="bg-muted/50 p-8 pt-4">
                    ...
                    <div className="flex gap-4">
                        <Button variant="outline" className="flex-1 h-16 text-xl" onClick={onClose}>
                        ...
                        >
                            {redeemMutation.isPending ? <Loader2 className="animate-spin h-8 w-8" /> : t('submit', { defaultValue: 'Submit' })}
                        </Button>
                    </div>
                </div>
            </DialogContent>
