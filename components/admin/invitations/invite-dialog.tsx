...
export default function InviteDialog({ open, onOpenChange, onSuccess }: InviteDialogProps) {
    ...
    const [unitPermissions, setUnitPermissions] = useState<Record<string, string[]>>({});
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                                                ...
                                                {AVAILABLE_PERMISSIONS.map(permId => (
                                                    <div key={permId} className="flex items-center space-x-2">
                                                        <Checkbox
                                                            ...
                                                            checked={(unitPermissions[unit.id] || []).includes(permId)}
                                                            onCheckedChange={() => toggleUnitPermission(unit.id, permId)}
                                                        />
                                                        <Label htmlFor={`unit-${unit.id}-${permId}`} className="cursor-pointer text-sm font-normal">
                                                            {tPermissions(permId)}
                                                        </Label>
                                                    </div>
                                                ))}
                    ...
                    <div>
                        <Label htmlFor="template" className="mb-2 block">{t('select_template')}</Label>
                    ...
                    {!selectedTemplate && (
                        <>
                            <div>
                                <Label htmlFor="subject" className="mb-2 block">{t('subject')}</Label>
                                <Input
                                    id="subject"
                                    value={customSubject}
                            ...
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground mt-1">
                                    {tTemplates('variables_help', { link: '{link}', email: '{email}' })}
                                </p>
                            </div>
                        </>
                    )}
