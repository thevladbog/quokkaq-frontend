        ...
        const buildPath = (s: Service): string => {
            ...
            while (current) {
                ...
                if (current.parent) {
                    ...
                    current = allServices.find((srv: Service) => srv.id === current?.parentId);
                } else {
                    current = null;
                }
            }
...
function SlotConfigForm({ unitId, initialConfig, t }: { unitId: string, initialConfig: SlotConfig, t: TranslationFunction }) {
    ...
    const [workingDays, setWorkingDays] = useState<string[]>(initialConfig.workingDays || ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday']);

    const updateConfigMutation = useMutation({
        mutationFn: (data: SlotConfig) =>
...
function SlotCapacitiesForm({
    ...
    const timeSlots = useMemo(() => {
        const slots: string[] = [];
        if (!config?.startTime || !config?.endTime || !config?.intervalMinutes) return slots;
    ...
    const handleCapacitiesSave = () => {
        ...
        const existingOtherServices = initialCapacities.filter(c => c.serviceId !== selectedServiceId);
        const allUpdates = [...existingOtherServices, ...currentServiceUpdates];
...
function GenerationSection({ unitId, t }: { unitId: string, t: TranslationFunction }) {
    ...
    const generateMutation = useMutation({
        ...
        onError: (err: Error) => toast.error(t('generated_error', { defaultValue: 'Generation failed' }) + ': ' + err.message),
    });
...
function DayManagementSection({ unitId, t, services, getLocalizedServiceName }: { unitId: string, t: TranslationFunction, services: Service[], getLocalizedServiceName: (service: Service, servicesList?: Service[]) => string }) {
...
function DayEditor({
    ...
    const updateDayMutation = useMutation({
        mutationFn: (data: { isDayOff: boolean; slots: { serviceId: string; startTime: string; capacity: number }[] }) =>
            slotsApi.updateDay(unitId, details.date, data),
        onSuccess: () => {
            toast.success(t('day_saved', { defaultValue: 'Day schedule saved' }));
            onSaved();
        },
        onError: (err: Error) => toast.error(t('day_save_error', { defaultValue: 'Failed to save day' }) + ': ' + err.message),
    });
